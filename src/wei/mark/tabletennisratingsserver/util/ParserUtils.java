...
public class ParserUtils {
	public static final long FRESH_THRESHOLD = 1 * 24 * 60 * 60 * 1000;
	...
	public static boolean firstNamePartsEqual(String firstNameQuery,
			String firstName) {
		String[] querySplit = firstNameQuery.split(" ");
		String[] firstNameSplit = firstName.split(" ", querySplit.length + 1);

		if (querySplit.length > firstNameSplit.length)
			return false;

		for (int i = 0; i < querySplit.length; i++) {
			if (!querySplit[i].equals(firstNameSplit[i]))
				return false;
		}
		return true;
	}
